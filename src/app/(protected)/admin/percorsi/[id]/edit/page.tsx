...
export default function AdminPercorsiEditPage() {
  ...
  const [form, setForm] = useState({
    ...
    cost: '',
    media: [] as string[],
    items: [] as AdminPercorso['items'],
  });

  useEffect(() => {
    const fetchPercorso = async () => {
      try {
        const res = await fetch(`/api/admin/percorsi/${id}`);
        if (res.ok) {
          const percorso: AdminPercorso = await res.json();
          setForm({
            name: percorso.name || '',
            description: percorso.description || '',
            cost: String(percorso.cost ?? ''),
            media: percorso.media || [],
            items: Array.isArray(percorso.items) ? percorso.items : [],
          });
        } else {
          const data = await res.json();
          alert(data.error || 'Percorso non trovato.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(data.error || 'Errore nell\'aggiornamento del percorso.');
      }
    } catch (err) {
      console.error('Errore:', err);
      alert('Errore nell\'aggiornamento del percorso.');
    } finally {
      setLoading(false);
    }
  };
