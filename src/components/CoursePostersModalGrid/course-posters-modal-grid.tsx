...
export type CoursePosterItem = {
  id: string;
  image: string;
  /** "Mese Anno" già formattato (es. "Gennaio 2026") */
  displayTitle: string;
};
...
export function CoursePostersModalGrid({ posters }: { posters: CoursePosterItem[] }) {
  const [openId, setOpenId] = useState<string | null>(null);
  const active = openId ? posters.find((p) => p.id === openId) : null;
    ...
    <>
        ...
        {posters.map((poster) => (
          ...
          >
            ...
            <div className="relative aspect-137/160 w-full">
              <Image
                src={poster.image}
                alt={`Locandina ${poster.displayTitle}`}
                fill
      ...
      {active ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`Locandina ${active.displayTitle}`}
          onClick={close}
        >
          <div
          ...
          >
            ...
            <div className="flex h-full min-h-0 w-full flex-col bg-[rgba(250,250,252,0.98)]">
              ...
              <div className="relative min-h-0 w-full flex-1">
                <Image
                  src={active.image}
                  alt={`Locandina ${active.displayTitle}`}
                  fill
            ...
            <a
              href={whatsappCorsoUrl(`Calendario corsi ${active.displayTitle}`)}
              target="_blank"
